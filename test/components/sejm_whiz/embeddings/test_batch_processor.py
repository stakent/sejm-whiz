...
class TestBatchProcessor:
    ...
    def mock_config(self):
        ...
        return EmbeddingConfig(batch_size=2, max_workers=2, embedding_dim=768)
    ...
    def mock_processor(self, mock_config):
        ...
        with (
            patch(
                "sejm_whiz.embeddings.batch_processor.get_herbert_encoder"
            ) as mock_encoder,
            patch(
                "sejm_whiz.embeddings.batch_processor.get_bag_embeddings_generator"
            ) as mock_bag_gen,
            patch(
                "sejm_whiz.embeddings.batch_processor.get_similarity_calculator"
            ) as mock_sim_calc,
        ):
            # Mock encoder
    ...
    async def test_process_embedding_batch_simple(self, mock_processor):
        ...
        result = await mock_processor.process_embedding_batch(
            texts, use_threading=False
        )
    ...
    async def test_process_embedding_batch_with_threading(self, mock_processor):
        ...
        with patch.object(
            mock_processor, "_process_embeddings_parallel"
        ) as mock_parallel:
            mock_parallel.return_value = [
                Mock(embedding=np.random.randn(768)) for _ in range(4)
            ]

            result = await mock_processor.process_embedding_batch(
                texts, use_threading=True
            )
    ...
    async def test_process_embedding_batch_with_error(self, mock_processor):
        ...
        mock_processor.encoder.encode_with_metadata.side_effect = ValueError(
            "Mock error"
        )
    ...
    async def test_process_bag_embeddings_batch_with_error(self, mock_processor):
        ...
        mock_processor.bag_generator.generate_bag_embeddings_batch.side_effect = (
    ...
    async def test_process_similarity_batch_with_error(self, mock_processor):
        ...
        mock_processor.similarity_calculator.find_most_similar.side_effect = (
    ...
    def test_get_processing_stats(self, mock_processor):
        ...
        mock_processor.processing_stats["embedding_batch"].append(
            {"job_id": "job1", "processing_time": 1.0, "success_rate": 1.0}
        )
        mock_processor.processing_stats["embedding_batch"].append(
            {"job_id": "job2", "processing_time": 2.0, "success_rate": 0.8}
        )
    ...
    def test_clear_cache(self, mock_processor):
        ...
        recent_result = Mock()
        recent_result.created_at = (
            current_time - 1800
        )  # 30 minutes old (should not be cleared)
...
async def test_batch_encode_texts_convenience():
    ...
    with patch(
        "sejm_whiz.embeddings.batch_processor.get_batch_processor"
...
async def test_batch_calculate_similarities_convenience():
    ...
    with patch(
        ...
        mock_processor.process_similarity_batch.assert_called_once_with(
            queries, candidates
        )
        assert result is not None
