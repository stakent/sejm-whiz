...
class TestSimilarityCalculator:
    ...
    def test_calculate_pairwise_similarities_no_labels(self, calculator, sample_embeddings):
    ...
    def test_cluster_by_similarity(self, calculator):
        ...
        clusters = calculator.cluster_by_similarity(embeddings, threshold=0.8, method='cosine')
    ...
    def test_generate_default_legal_weights(self, calculator):
        ...
        middle_start = 768 // 4
        middle_end = 3 * 768 // 4
        assert np.mean(weights[middle_start:middle_end]) > np.mean(weights[:middle_start])
