...
class TestPolishLegalNER:
    ...
    def test_extract_law_references(self):
        ...
        law_refs = [
            e for e in entities if e.entity_type == LegalEntityType.LAW_REFERENCE
        ]
    ...
    def test_extract_article_references(self):
        ...
        entities = self.ner.extract_legal_references(text)
        article_refs = [
            e for e in entities if e.entity_type == LegalEntityType.ARTICLE_REFERENCE
        ]
    ...
    def test_extract_paragraph_references(self):
        ...
        entities = self.ner.extract_legal_references(text)
        para_refs = [
            e for e in entities if e.entity_type == LegalEntityType.PARAGRAPH_REFERENCE
        ]
    ...
    def test_extract_court_names(self):
        ...
        entities = self.ner.extract_legal_references(text)
        court_refs = [
            e for e in entities if e.entity_type == LegalEntityType.COURT_NAME
        ]
    ...
    def test_extract_legal_persons(self):
        ...
        entities = self.ner.extract_legal_references(text)
        legal_persons = [
            e for e in entities if e.entity_type == LegalEntityType.LEGAL_PERSON
        ]
