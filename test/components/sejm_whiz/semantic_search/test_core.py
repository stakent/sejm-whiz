...
class TestSemanticSearchService:
    ...
    @pytest.fixture
    def search_service(
        self, mock_search_engine, mock_indexer, mock_ranker, mock_cross_register_matcher
    ):
    ...
    def test_get_search_statistics(
        self, search_service, mock_indexer, mock_ranker, mock_cross_register_matcher
    ):
        ...
        assert (
            stats["search_engine"]["embedding_model"] == "allegro/herbert-klej-cased-v1"
        )
...
def test_process_search_query():
    ...
    with patch(
        "sejm_whiz.semantic_search.core.get_semantic_search_service"
    ) as mock_get_service:
        mock_service = Mock()
        mock_result = Mock()
