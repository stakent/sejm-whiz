...
class TestCrossRegisterMatcher:
    ...
    def test_match_registers_direct_mappings(self, matcher, legal_text, parliamentary_text):
        """Test direct term mapping between registers."""
        with patch('sejm_whiz.semantic_search.cross_register.process_legal_document') as mock_process:
            mock_processed = {"clean_text": legal_text}
            mock_process.return_value = mock_processed
            
            with patch('sejm_whiz.semantic_search.cross_register.normalize_legal_text') as mock_normalize:
        ...
        assert any("w rozumieniu niniejszej ustawy" in formal and "według tej ustawy" in informal 
    ...
    def test_match_registers_semantic_similarity(self, matcher, legal_text, parliamentary_text, mock_similarity_calculator):
    ...
    def test_match_registers_structural_patterns(self, matcher, legal_text, parliamentary_text):
        ...
        structural_matches = [m for m in matches if m.match_type == "structural_pattern"]
        assert len(structural_matches) > 0
        
        # Check for article reference pattern
        article_matches = [m for m in structural_matches if "article_reference" in m.key_terms]
        assert len(article_matches) > 0
        
        # Check for legal obligation pattern
        obligation_matches = [m for m in structural_matches if "legal_obligation" in m.key_terms]
        assert len(obligation_matches) > 0
    
    def test_confidence_threshold_filtering(self, matcher, legal_text, parliamentary_text):
    ...
    def test_similarity_threshold_filtering(self, matcher, legal_text, parliamentary_text):
    ...
    def test_similarity_calculation_failure(self, matcher, legal_text, parliamentary_text, mock_similarity_calculator):
        """Test handling of similarity calculation failures."""
        # Mock similarity calculation failure
        mock_similarity_calculator.calculate_similarity.side_effect = Exception("Similarity failed")
