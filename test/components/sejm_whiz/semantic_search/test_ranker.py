...
class TestResultRanker:
    ...
    def test_rank_with_temporal_boost(self, ranker, sample_search_results):
        ...
        assert (
            recent_result.search_metadata["temporal_score"]
            > old_result.search_metadata["temporal_score"]
        )
    ...
    def test_rank_with_document_type_boost(self, ranker, sample_search_results):
        ...
        constitution_result = next(
            r for r in ranked_results if r.document.document_type == "ustawa"
        )
        assert constitution_result.search_metadata["document_type_weight"] == 1.2

        # Code (kodeks) should have weight 1.3
        code_result = next(
            r for r in ranked_results if r.document.document_type == "kodeks"
        )
    ...
    def test_rank_with_legal_domain_boost(self, ranker, sample_search_results):
        ...
        const_result = next(
            ...
            if r.document.legal_domain == "prawo konstytucyjne"
        )
    ...
    def test_rank_composite(self, ranker, sample_search_results):
        ...
        multi_act_result = next(
            r for r in ranked_results if r.document.affects_multiple_acts
        )
        assert (
            multi_act_result.search_metadata["amendment_boost"]
            >= ranker.config.multi_act_amendment_boost
        )
    ...
    def test_custom_weights(self, ranker, sample_search_results):
        ...
        ranked_results = ranker.rank_results(
            sample_search_results, query="test query", custom_weights=custom_weights
        )

        # Custom weights should be applied
        for result in ranked_results:
            # Document type weights should be multiplied by custom weight
            base_doc_weight = ranker.config.document_type_weights.get(
                result.document.document_type, 1.0
            )
            expected_doc_weight = base_doc_weight * custom_weights["document_type"]
            assert (
                abs(
                    result.search_metadata["document_type_weight"] - expected_doc_weight
                )
                < 0.001
            )
    ...
    def test_score_clamping(self, ranker, sample_search_results):
        ...
        for result in ranked_results:
            assert (
                ranker.config.min_score
                <= result.similarity_score
                <= ranker.config.max_score
            )
