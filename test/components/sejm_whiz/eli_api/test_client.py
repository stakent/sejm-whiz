...
class TestEliApiClient:
    ...
    async def test_make_request_rate_limit_error(self):
        ...
        def mock_get_side_effect(*args, **kwargs):
            ...
            if call_count == 1:
                ...
                mock_response.raise_for_status.return_value = None  # Don't raise for 429
                ...
                mock_response_success.raise_for_status.return_value = None  # Don't raise for 200
    ...
    async def test_get_document_not_found(self, client):
        ...
        with patch.object(client, '_make_request', side_effect=EliNotFoundError("Not found")):
            with pytest.raises(EliNotFoundError):
    ...
    async def test_batch_get_documents_size_limit(self, client):
        ...
        with pytest.raises(EliValidationError, match="Batch size 51 exceeds maximum allowed 50"):
    ...
    async def test_batch_get_documents_custom_limits(self, client):
        ...
        with patch.object(client, 'get_document', side_effect=mock_get_document):
            ...
            result = await client.batch_get_documents(eli_ids, max_batch_size=5, max_concurrent=2)
            assert len(result) == 3
            assert all(doc is not None for doc in result)
            
            # Test batch size exceeding custom limit
            with pytest.raises(EliValidationError, match="Batch size 3 exceeds maximum allowed 2"):
    ...
    async def test_batch_get_documents_input_validation(self, client):
        ...
        with pytest.raises(EliValidationError, match="max_concurrent must be at least 1"):
    ...
    async def test_batch_get_documents_duplicate_handling(self, client):
        ...
        eli_ids = ["pl/test/2023/1", "pl/test/2023/1", "pl/test/2023/2"]  # Has duplicate
        call_count = 0
