...
class RedisJobQueue:
    ...
    def enqueue(
        ...
        try:
            ...
            self._client.setex(
                job_key, self.config.result_ttl, json.dumps(job.to_dict())
            )
            ...
            logger.info(
                f"Enqueued job {job_id} with task '{task_name}' and priority {priority.value}"
            )
            return job_id
    ...
    def complete_job(self, job_id: str, result: Any = None) -> bool:
        ...
        try:
            ...
            if result is not None:
                ...
                self._client.setex(
                    result_key, self.config.result_ttl, json.dumps(result)
                )
    ...
    def fail_job(self, job_id: str, error: str, retry: bool = True) -> bool:
        ...
        try:
            ...
            if retry and job.retry_count <= job.max_retries:
                ...
                logger.info(
                    f"Job {job_id} failed, retrying (attempt {job.retry_count}/{job.max_retries})"
                )
            else:
