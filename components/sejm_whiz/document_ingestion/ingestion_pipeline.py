...
from sejm_whiz.redis import get_redis_cache, get_redis_queue, JobPriority, JobStatus

from .config import get_ingestion_config, DocumentIngestionConfig
from .eli_client import ELIClient, get_eli_client
...
class DocumentIngestionPipeline:
        ...
        try:
            ...
            async with ELIClient(self.config) as eli_client:
                ...
                recent_docs = await eli_client.get_recent_documents(
                    days=days,
                    document_types=self.config.legal_document_types
                )
        ...
        for i in range(0, len(eli_ids), batch_size):
            ...
            for eli_id, result in zip(batch, batch_results):
                if isinstance(result, Exception):
                    ...
                    results.append({"status": "error", "eli_id": eli_id, "error": str(result)})
                else:
    ...
    async def _process_documents_batch(self, eli_client: ELIClient, documents: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        for i in range(0, len(documents), self.config.batch_size):
            ...
            logger.info(f"Processing batch {i//self.config.batch_size + 1} ({len(batch)} documents)")
            
            # Process documents in parallel
            if self.config.parallel_workers > 1:
                semaphore = asyncio.Semaphore(self.config.parallel_workers)
                tasks = [
                    self._process_single_document_with_semaphore(eli_client, doc, semaphore)
                    for doc in batch
                ]
            ...
            for doc, result in zip(batch, batch_results):
                if isinstance(result, Exception):
                    logger.error(f"Exception processing document {doc.get('eli_id')}: {result}")
    ...
    async def _process_single_document_with_semaphore(self, eli_client: ELIClient, doc: Dict[str, Any], semaphore: asyncio.Semaphore) -> Dict[str, Any]:
    ...
    async def _process_single_document(self, eli_client: ELIClient, doc: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if doc.get('published_date'):
                processed_doc.published_at = datetime.fromisoformat(doc['published_date'])
            ...
            return {"status": "ingested", "eli_id": eli_id, "document_id": str(stored_doc.id)}
    ...
    async def enqueue_ingestion_job(self, eli_ids: List[str], priority: JobPriority = JobPriority.NORMAL) -> str:
...
def get_ingestion_pipeline(config: Optional[DocumentIngestionConfig] = None) -> DocumentIngestionPipeline:
