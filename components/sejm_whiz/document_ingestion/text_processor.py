...
class TextProcessor:
    ...
    def __init__(self, config: Optional[DocumentIngestionConfig] = None):
        ...
        self.legal_patterns = {
            # Document types
            "ustawa": re.compile(
                r"ustawa\s+z\s+dnia\s+(\d{1,2}\s+\w+\s+\d{4})", re.IGNORECASE
            ),
            "kodeks": re.compile(r"kodeks\s+(\w+)", re.IGNORECASE),
            "rozporządzenie": re.compile(
                r"rozporządzenie\s+(.+?)\s+z\s+dnia\s+(\d{1,2}\s+\w+\s+\d{4})",
                re.IGNORECASE,
            ),
            "konstytucja": re.compile(
                r"konstytucja\s+rzeczypospolitej\s+polskiej", re.IGNORECASE
            ),
            # Legal structure
            ...
            "law_reference": re.compile(
                r"ustawy?\s+z\s+dnia\s+(\d{1,2}\s+\w+\s+\d{4})", re.IGNORECASE
            ),
            "article_reference": re.compile(
                r"art\.\s*(\d+[a-z]?)\s+ustawy", re.IGNORECASE
            ),
            "regulation_reference": re.compile(
                r"rozporządzenia?\s+(.+?)\s+z\s+dnia\s+(\d{1,2}\s+\w+\s+\d{4})",
                re.IGNORECASE,
            ),
            # Dates
            "date": re.compile(r"(\d{1,2})\s+(\w+)\s+(\d{4})", re.IGNORECASE),
            "effective_date": re.compile(
                r"wchodzi\s+w\s+życie\s+(.+?)(?:\.|$)", re.IGNORECASE
            ),
            # Amendments
            "amendment_marker": re.compile(
                r"nowelizacja|zmiana|uchylenie|dodanie|skreślenie", re.IGNORECASE
            ),
            "omnibus_marker": re.compile(
                r"ustawa\s+o\s+zmianie\s+(.+?)\s+oraz\s+(.+)", re.IGNORECASE
            ),
        }
    ...
    def _extract_structure(self, content: str) -> LegalStructure:
        ...
        structure = LegalStructure(
            chapters=[], articles=[], paragraphs=[], sections=[], references=[]
        )
        ...
        for line in lines[:10]:
            ...
            if line and len(line) > 10:
                ...
                if any(
                    ...
                    for keyword in ["ustawa", "kodeks", "rozporządzenie", "konstytucja"]
                ):
    ...
    def _calculate_quality_score(
        self, content: str, title: str, eli_id: Optional[str]
    ) -> float:
        ...
        content_lower = content.lower()
        if any(
            doc_type in content_lower
            for doc_type in ["ustawa", "kodeks", "rozporządzenie"]
        ):
            score += 0.2
    ...
    def validate_document(self, document: ProcessedDocument) -> Tuple[bool, List[str]]:
        ...
        if not document.content or len(document.content) < self.config.min_text_length:
            errors.append(
                f"Content too short (min: {self.config.min_text_length} chars)"
            )

        if len(document.content) > self.config.max_text_length:
            errors.append(
                f"Content too long (max: {self.config.max_text_length} chars)"
            )
        ...
        if document.quality_score < self.config.min_quality_score:
            errors.append(
                f"Quality score too low: {document.quality_score:.2f} < {self.config.min_quality_score}"
            )
