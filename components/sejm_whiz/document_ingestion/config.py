...
class DocumentIngestionConfig(BaseSettings):
    ...
    # ELI API settings
    eli_api_base_url: str = Field(
        default="https://eli.gov.pl/api", env="ELI_API_BASE_URL"
    )
    eli_api_timeout: int = Field(default=30, env="ELI_API_TIMEOUT")
    eli_api_max_retries: int = Field(default=3, env="ELI_API_MAX_RETRIES")
    eli_api_rate_limit: int = Field(
        default=10, env="ELI_API_RATE_LIMIT"
    )  # requests per second

    # Document processing settings
    max_document_size: int = Field(default=10485760, env="MAX_DOCUMENT_SIZE")  # 10MB
    supported_formats: List[str] = Field(
        default=["text/plain", "text/html", "application/pdf"], env="SUPPORTED_FORMATS"
    )
    ...
    backup_processed_documents: bool = Field(
        default=True, env="BACKUP_PROCESSED_DOCUMENTS"
    )
