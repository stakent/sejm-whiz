...
def upgrade() -> None:
    ...
    op.create_table('legal_amendments',
        sa.Column('id', postgresql.UUID(as_uuid=True), primary_key=True, server_default=sa.text('uuid_generate_v4()')),
    ...
    op.create_table('cross_references',
        ...
        sa.Column('target_document_id', postgresql.UUID(as_uuid=True), sa.ForeignKey('legal_documents.id'), nullable=False),
    ...
    op.create_index('idx_legal_documents_published', 'legal_documents', ['published_at'])
    op.create_index('idx_legal_documents_embedding', 'legal_documents', ['embedding'], postgresql_using='ivfflat')
    ...
    op.create_index('idx_embeddings_model', 'document_embeddings', ['model_name', 'model_version'])
    op.create_index('idx_embeddings_vector', 'document_embeddings', ['embedding'], postgresql_using='ivfflat')
