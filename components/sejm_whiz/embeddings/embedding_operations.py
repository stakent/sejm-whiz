...
class EmbeddingOperations:
    ...
    async def generate_document_embedding(self, document_id: str) -> Optional[EmbeddingResult]:
        ...
        try:
            ...
            logger.error(f"Failed to generate embedding for document {document_id}: {e}")
            return None
    
    async def generate_embeddings_batch(self, document_ids: List[str]) -> Dict[str, Optional[EmbeddingResult]]:
        ...
        for i in range(0, len(document_ids), batch_size):
            ...
            batch_tasks = [
                self.generate_document_embedding(doc_id) 
            ...
            for doc_id, result in zip(batch_ids, batch_results):
                if isinstance(result, Exception):
                    logger.error(f"Exception generating embedding for {doc_id}: {result}")
                    results[doc_id] = None
                else:
                    results[doc_id] = result
        
        success_count = sum(1 for r in results.values() if r is not None)
        logger.info(f"Generated {success_count}/{len(document_ids)} embeddings successfully")
    ...
    async def find_similar_by_document(self, 
                                     document_id: str,
                                     limit: int = 10,
                                     threshold: float = None) -> List[Dict[str, Any]]:
        ...
        if source_doc.embedding:
            ...
            if not embedding_result:
                logger.error(f"Failed to generate embedding for source document: {document_id}")
                return []
        ...
        filtered_docs = [
            (doc, distance) for doc, distance in similar_docs 
    ...
    async def batch_similarity_search(self, 
        ...
        for query, search_result in zip(queries, search_results):
            if isinstance(search_result, Exception):
                logger.error(f"Search failed for query '{query[:50]}...': {search_result}")
    ...
    async def enqueue_embedding_generation(self, 
        ...
        logger.info(f"Enqueued embedding generation job {job_id} for {len(document_ids)} documents")
        return job_id
    ...
    async def process_embedding_job(self, document_ids: List[str]) -> Dict[str, Any]:
        ...
        logger.info(f"Processing embedding generation job for {len(document_ids)} documents")
        ...
        try:
            ...
            logger.info(f"Completed embedding generation job: {success_count}/{len(document_ids)} successful")
    ...
    async def update_embeddings_for_new_documents(self, hours_back: int = 24) -> Dict[str, Any]:
        ...
        documents_without_embeddings = await self.db_operations.get_documents_without_embeddings(
            limit=100,
        ...
        logger.info(f"Updated embeddings for {success_count}/{len(document_ids)} documents")
    ...
    def get_embedding_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'device': str(self.embedder.device) if hasattr(self.embedder, 'device') else 'unknown'
        }
...
def get_embedding_operations(config: Optional[EmbeddingConfig] = None) -> EmbeddingOperations:
