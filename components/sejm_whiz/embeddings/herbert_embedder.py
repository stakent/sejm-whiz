...
class HerBERTEmbedder:
    ...
    def __init__(self, config: Optional[EmbeddingConfig] = None):
        ...
        self.polish_legal_patterns = {
            ...
            'definition_marker': re.compile(r'oznacza|rozumie się|należy przez to rozumieć', re.IGNORECASE),
            'amendment_marker': re.compile(r'zmienia się|uchyla się|dodaje się|skreśla się', re.IGNORECASE)
        }
    ...
    def _process_batch(self, texts: List[str]) -> List[EmbeddingResult]:
        ...
        for text in texts:
            try:
                ...
                # Count tokens
                token_count = len(self.tokenizer.encode(processed_text, truncation=True, max_length=self.config.max_length))
    ...
    def embed_legal_document(self, 
        ...
        for section_type, section_text in sections.items():
            if section_text:
                ...
                section_weight = self.config.legal_section_weights.get(section_type, 1.0)
        ...
        token_count = len(self.tokenizer.encode(total_text, truncation=True, max_length=self.config.max_length))
    ...
    def _extract_legal_sections(self, title: str, content: str, document_type: str) -> Dict[str, str]:
        ...
        # Extract definitions
        definition_matches = self.polish_legal_patterns['definition_marker'].finditer(content)
        definitions = []
        for match in definition_matches:
            start = match.start()
            definition_text = content[start:start+200]  # 200 chars after definition marker
        ...
        if self.polish_legal_patterns['amendment_marker'].search(content):
            amendment_matches = self.polish_legal_patterns['amendment_marker'].finditer(content)
            amendments = []
