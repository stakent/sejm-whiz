...
class EmbeddingConfig(BaseSettings):
    ...
    pooling_strategy: str = Field(
        default="mean", env="EMBEDDING_POOLING"
    )  # mean, cls, max

    # Text preprocessing
    normalize_text: bool = Field(default=True, env="EMBEDDING_NORMALIZE_TEXT")
    remove_special_chars: bool = Field(
        default=False, env="EMBEDDING_REMOVE_SPECIAL_CHARS"
    )
    lowercase: bool = Field(
        default=False, env="EMBEDDING_LOWERCASE"
    )  # HerBERT is cased
    ...
    chunk_strategy: str = Field(
        default="sentence", env="EMBEDDING_CHUNK_STRATEGY"
    ...
    compile_model: bool = Field(
        default=False, env="EMBEDDING_COMPILE_MODEL"
    )  # PyTorch 2.0 compile
    ...
    similarity_threshold: float = Field(
        default=0.8, env="EMBEDDING_SIMILARITY_THRESHOLD"
    )
