...
from .herbert_encoder import get_herbert_encoder
from .bag_embeddings import get_bag_embeddings_generator
...
class BatchProcessor:
    ...
    async def process_embedding_batch(
        self, texts: List[str], job_id: str = None, use_threading: bool = True
    ) -> BatchResult:
        """
        ...
        logger.info(
            f"Completed batch {job_id}: {success_count}/{len(texts)} successful in {processing_time:.2f}s"
        )
        return batch_result

    async def process_bag_embeddings_batch(
        self,
        documents: List[Dict[str, str]],
        job_id: str = None,
        use_tf_idf: bool = True,
        use_legal_weighting: bool = True,
    ) -> BatchResult:
        """
        ...
        logger.info(
            f"Processing bag embeddings batch {job_id} with {len(documents)} documents"
        )
        ...
        try:
            ...
            logger.error(
                f"Bag embeddings batch processing failed for job {job_id}: {e}"
            )
            errors.append(str(e))
            results = []
            success_count = 0
        ...
        batch_result = BatchResult(
            ...
            metadata={
                ...
                "avg_processing_time": processing_time / len(documents)
                if documents
                else 0,
        ...
        logger.info(
            f"Completed bag embeddings batch {job_id}: {success_count}/{len(documents)} successful"
        )
        return batch_result

    async def process_similarity_batch(
        self,
        ...
        logger.info(
            f"Processing similarity batch {job_id} with {len(query_embeddings)} queries"
        )
        ...
        batch_result = BatchResult(
            ...
            metadata={
                ...
                "method": method,
                "top_k": top_k,
                "avg_query_time": processing_time / len(query_embeddings)
                if query_embeddings
                else 0,
            },
        )
        ...
        logger.info(
            f"Completed similarity batch {job_id}: {success_count}/{len(query_embeddings)} successful"
        )
        return batch_result
...
async def batch_generate_bag_embeddings(
    documents: List[Dict[str, str]], config: Optional[EmbeddingConfig] = None
) -> BatchResult:
...
async def batch_calculate_similarities(
    ...
    processor = get_batch_processor(config)
    return await processor.process_similarity_batch(
