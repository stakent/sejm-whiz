...
class BagEmbeddingsGenerator:
    ...
    def __init__(self, config: Optional[EmbeddingConfig] = None):
        ...
        self.legal_term_patterns = {
            "act_reference": re.compile(
                r"\bustawa\s+z\s+dnia\s+[\d\s\w]+", re.IGNORECASE
            ),
            ...
            "legal_entity": re.compile(
                r"minister|rząd|parlament|sejm|senat|prezydent", re.IGNORECASE
            ),
            "legal_procedure": re.compile(
                r"procedura|postępowanie|proces|tryb", re.IGNORECASE
            ),
        }
    ...
    def generate_bag_embedding(
        ...
        document_embedding = self._create_weighted_embedding(
    ...
    def generate_bag_embeddings_batch(
        self,
        documents: List[Dict[str, str]],
        use_tf_idf: bool = True,
        use_legal_weighting: bool = True,
    ) -> List[BagEmbeddingResult]:
        """
    ...
    def _calculate_token_weights(
        self, tokens: List[str], text: str, use_tf_idf: bool, use_legal_weighting: bool
    ) -> List[float]:
    ...
    def _create_weighted_embedding(
        self, token_embeddings: List[np.ndarray], token_weights: List[float]
    ) -> np.ndarray:
    ...
    def get_token_importance_analysis(
        self, result: BagEmbeddingResult
    ) -> Dict[str, Any]:
        ...
        analysis = {
            ...
            "vocabulary_diversity": result.vocabulary_size / len(result.tokens)
            if result.tokens
            else 0,
...
def get_bag_embeddings_generator(
