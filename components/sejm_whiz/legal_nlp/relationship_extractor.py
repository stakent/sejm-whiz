...
class LegalRelationshipExtractor:
    ...
    def _init_legal_vocabulary(self):
        ...
        self.legal_vocabulary = {
            'modifiers': [
                'bezwarunkowo', 'warunkowo', 'tymczasowo', 'ostatecznie',
                'natychmiast', 'niezwłocznie', 'stopniowo', 'częściowo',
                'całkowicie', 'w pełni', 'ograniczenie', 'wyjątkowo'
            ],
            'temporal_markers': [
                'od dnia', 'do dnia', 'w dniu', 'z dniem', 'w terminie',
                'przed upływem', 'po upływie', 'w ciągu', 'nie później niż',
                'nie wcześniej niż', 'równocześnie', 'jednocześnie'
            ],
            'legal_operators': [
                'zgodnie z', 'na podstawie', 'w związku z', 'w świetle',
                'z zastrzeżeniem', 'z wyłączeniem', 'z wyjątkiem',
                'pod rygorem', 'w granicach', 'w zakresie'
            ]
        }
    ...
    def _remove_overlapping_entities(self, entities: List[LegalEntity]) -> List[LegalEntity]:
    ...
    def _enrich_entities(self, entities: List[LegalEntity], text: str) -> List[LegalEntity]:
    ...
    def extract_relationships(self, text: str, entities: Optional[List[LegalEntity]] = None) -> List[LegalRelationship]:
    ...
    def _find_matching_entity(self, text: str, entities: List[LegalEntity]) -> Optional[LegalEntity]:
    ...
    def _extract_relationship_context(self, text: str, start: int, end: int, window: int = 150) -> str:
    ...
    def _calculate_relationship_confidence(self, match, context: str, rel_type: RelationshipType,
                                         source_entity: LegalEntity, target_entity: LegalEntity) -> float:
        ...
        entity_boost = 0
        if source_entity.entity_type != 'concept' and target_entity.entity_type != 'concept':
            entity_boost = 0.1
        ...
        confidence = base_confidence + pattern_boost + context_boost + entity_boost + type_boost
        return max(0.0, min(1.0, confidence))
    
    def analyze_relationship_network(self, relationships: List[LegalRelationship]) -> Dict[str, Any]:
        ...
        central_entities = sorted(entity_frequency.items(), key=lambda x: x[1], reverse=True)[:10]
        ...
        network_metrics = {
            ...
            'relationship_density': len(relationships) / max(1, len(entity_frequency) ** 2),
            'average_confidence': sum(rel.confidence for rel in relationships) / len(relationships)
        }
    ...
    def _analyze_relationship_patterns(self, relationships: List[LegalRelationship]) -> Dict[str, Any]:
            ...
            for connection in connections:
                ...
                if target in adjacency:
                    ...
                    patterns['chains'].append({
                        'chain': [entity, target],
                        'extended_to': list(adjacency[target])[:3]  # Show next 3 connections
                    })
    ...
    def perform_comprehensive_relationship_analysis(self, text: str) -> Dict[str, Any]:
        ...
        return {
            ...
            'relationships': {
                ...
                'by_type': {rel_type.value: len([r for r in relationships if r.relationship_type == rel_type])
