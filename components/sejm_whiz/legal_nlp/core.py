...
class LegalNLPAnalyzer:
    ...
    def _calculate_concept_confidence(
        self, match, context: str, concept_type: LegalConceptType
    ) -> float:
        ...
        confidence = min(
            1.0, base_confidence + legal_term_boost + type_boost + length_penalty
        )
        return max(0.0, confidence)

    def _remove_overlapping_concepts(
        ...
        for concept in sorted_concepts:
            ...
            for existing in filtered_concepts:
                if (
                    concept.start_pos < existing.end_pos
                    and concept.end_pos > existing.start_pos
                ):
                    # There's an overlap
            ...
            for concept2 in concepts[i + 1 :]:
                ...
                if relation_type:
                    confidence = self._calculate_relation_confidence(
                        concept1, concept2, relation_type
                    )
                    ...
                    relation = SemanticRelation(
                        ...
                        confidence=confidence,
                        evidence_text=self._extract_relation_evidence(
                            concept1, concept2
                        ),
                    )
    ...
    def _determine_relation_type(
        self, concept1: LegalConcept, concept2: LegalConcept
    ) -> Optional[str]:
    ...
    def _calculate_relation_confidence(
        self, concept1: LegalConcept, concept2: LegalConcept, relation_type: str
    ) -> float:
        ...
        confidence = (
            base_confidence + concept_confidence_boost + type_boost - distance_penalty
        )
        return max(0.0, min(1.0, confidence))

    def _extract_relation_evidence(
        self, concept1: LegalConcept, concept2: LegalConcept
    ) -> str:
    ...
    def _calculate_complexity_score(
        ...
        discourse_score = min(
            1.0, len([t for t, m in discourse_markers.items() if m]) / 5
        )
