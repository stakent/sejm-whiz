...
class EliApiClient:
        ...
        for attempt in range(self.config.max_retries):
            try:
                ...
                if attempt == self.config.max_retries - 1:
                    raise EliApiError(f"HTTP error after {self.config.max_retries} attempts: {e}")
                ...
                if attempt == self.config.max_retries - 1:
                    raise EliApiError(f"Request error after {self.config.max_retries} attempts: {e}")
    ...
    async def search_documents(self, 
                              ...
                              limit: int = 100,
                              offset: int = 0) -> DocumentSearchResult:
    ...
    async def get_document_content(self, eli_id: str, format: str = "html") -> str:
        ...
        if format not in valid_formats:
            raise EliValidationError(f"Invalid format: {format}. Must be one of {valid_formats}")
    ...
    async def get_recent_documents(self, days: int = 7, 
                                  document_types: Optional[List[str]] = None) -> List[LegalDocument]:
        ...
        for doc_type in document_types:
            try:
                ...
                logger.info(f"Found {len(result.documents)} recent {doc_type} documents")
    ...
    async def batch_get_documents(self, 
                                 eli_ids: List[str], 
                                 max_batch_size: int = 50,
                                 max_concurrent: int = 10) -> List[Union[LegalDocument, None]]:
        ...
        if len(eli_ids) > max_batch_size:
            raise EliValidationError(f"Batch size {len(eli_ids)} exceeds maximum allowed {max_batch_size}")
        
        if max_concurrent < 1:
            raise EliValidationError("max_concurrent must be at least 1")
        
        # Validate individual ELI IDs
        invalid_ids = [eli_id for eli_id in eli_ids if not isinstance(eli_id, str) or not eli_id.strip()]
        if invalid_ids:
        ...
        if len(unique_eli_ids) != len(eli_ids):
            logger.info(f"Removed {len(eli_ids) - len(unique_eli_ids)} duplicate ELI IDs from batch")
        
        logger.info(f"Starting batch fetch of {len(unique_eli_ids)} documents with max_concurrent={max_concurrent}")
        ...
        logger.info(f"Batch fetched {successful_count}/{len(eli_ids)} documents successfully")
