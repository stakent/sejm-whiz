...
class LegalDocument(BaseModel):
    ...
    repeal_date: Optional[datetime] = Field(None, description="Date when document was repealed")
    
    publisher: Optional[str] = Field(None, description="Publishing authority")
    journal_reference: Optional[str] = Field(None, description="Official journal reference")
    ...
    subject_areas: List[str] = Field(default_factory=list, description="Legal subject areas")
    
    amending_documents: List[str] = Field(default_factory=list, description="ELI IDs of amending documents")
    amended_documents: List[str] = Field(default_factory=list, description="ELI IDs of amended documents")
    ...
    def validate_journal_year(cls, v):
        ...
        if v is not None:
            ...
            if v < 1918 or v > current_year + 1:  # Poland regained independence in 1918
                raise ValueError(f"Journal year must be between 1918 and {current_year + 1}")
        return v
    ...
    def from_api_response(cls, data: Dict[str, Any]) -> 'LegalDocument':
        ...
        if data.get('repeal_date'):
            try:
                repeal_date = datetime.fromisoformat(data['repeal_date'].replace('Z', '+00:00'))
            except (ValueError, AttributeError):
                pass
...
class Amendment(BaseModel):
    ...
    published_date: Optional[datetime] = Field(None, description="Amendment publication date")
    effective_date: Optional[datetime] = Field(None, description="Date when amendment becomes effective")
    
    affected_articles: List[str] = Field(default_factory=list, description="List of affected articles")
    affected_paragraphs: List[str] = Field(default_factory=list, description="List of affected paragraphs")
...
class DocumentStatistics(BaseModel):
    ...
    recent_documents_count: int = Field(0, description="Documents published in last 30 days")
...
class MultiActAmendment(BaseModel):
    ...
    affected_acts: List[str] = Field(..., description="List of ELI IDs of affected acts")
    ...
    cross_references: List[str] = Field(default_factory=list, description="Cross-references between acts")
    impact_assessment: Optional[str] = Field(None, description="Impact assessment summary")
