...
from .models import LegalDocument, Amendment, MultiActAmendment
...
class LegalTextParser:
    ...
    def __init__(self):
        ...
        self.chapter_pattern = re.compile(r'Rozdział\s+([IVXLCDM]+)\s*\.?\s*(.*)', re.IGNORECASE)
        self.article_pattern = re.compile(r'Art\.\s*(\d+[a-z]?)\s*\.?\s*(.*)', re.IGNORECASE)
        ...
        self.article_reference_pattern = re.compile(r'art\.\s*(\d+[a-z]?)', re.IGNORECASE)
    ...
    def _extract_paragraphs(self, lines: List[str], start_index: int) -> Tuple[List[Paragraph], int]:
    ...
    def _extract_points(self, lines: List[str], start_index: int) -> Tuple[List[Point], int]:
        ...
        while i < len(lines):
            ...
            if (self.article_pattern.match(line) or 
                self.paragraph_pattern.match(line)):
                break
    ...
    def _extract_subpoints(self, lines: List[str], start_index: int) -> Tuple[List[Subpoint], int]:
        ...
        while i < len(lines):
            ...
            if (self.article_pattern.match(line) or 
                self.paragraph_pattern.match(line) or
                self.point_pattern.match(line)):
                break
...
class MultiActAmendmentDetector:
    ...
    def detect_multi_act_amendments(self, document: LegalDocument, content: str) -> Optional[MultiActAmendment]:
